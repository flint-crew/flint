...
def flag_ms_aoflagger(ms: MS, container: Path) -> MS:
    """Create and run an aoflagger command in a container. Checks are made
    to ensure that any channels completely flagged across all timesteps per
    baseline before aoflagger is run remain flagged after aoflagger has been run.
    This is to avoid unflagging in circumstance where uncalibrated data were
    flagged but otherwise unmodified when applying calibration solutions.

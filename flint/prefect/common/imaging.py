...
def task_wsclean_imager(
    in_ms: ApplySolutions | MS | list[MS],
    ...
    Args:
        in_ms (Union[ApplySolutions, MS, list[MS]]): The measurement set that will be imaged
    ...
    if isinstance(in_ms, list):
        assert len(in_ms) > 0, "Empty list of measurement sets passed to wsclean imager"
        ms = in_ms
    else:
    ...
    logger.info(f"wsclean imager {ms=}")
